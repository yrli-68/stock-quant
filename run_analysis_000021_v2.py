#!/usr/bin/env python3
"""深科技(000021) 全策略量化分析 v2 — 取消综合(个股)，权重重新分配"""
...
print('='*80)
print(f'  {name}({code}) 全策略量化分析 v2')
print(f'  取消综合(个股)策略，权重重新分配')
print('='*80)
...
# 新权重：个股各20%（合计80%），指数各6.67%（合计20%）
STOCK_WEIGHT = 0.20     # 均线交叉/MACD/RSI/布林带 各20%
INDEX_WEIGHT = 0.0667   # 动量分层/波动率择时/涨跌比确认 各6.67%
...
all_weights = [STOCK_WEIGHT, STOCK_WEIGHT, STOCK_WEIGHT, STOCK_WEIGHT,
               INDEX_WEIGHT, INDEX_WEIGHT, INDEX_WEIGHT]
...
# 综合判断
if buy_count >= 5:
    verdict = '强烈买入'; v_color = 'green'
elif buy_count >= 3:
    verdict = '买入'; v_color = 'green'
elif buy_count >= 2 and sell_count <= 1:
    verdict = '偏多买入'; v_color = 'green'
elif sell_count >= 5:
    verdict = '强烈卖出'; v_color = 'red'
elif sell_count >= 3:
    verdict = '卖出'; v_color = 'red'
elif sell_count >= 2 and buy_count <= 1:
    verdict = '偏空卖出'; v_color = 'red'
elif buy_count >= 1:
    verdict = '谨慎偏多'; v_color = 'amber'
else:
    verdict = '观望'; v_color = 'amber'
...
with open('/workspace/stock_quant/data/000021_v2_result.json', 'w', encoding='utf-8') as f:
    json.dump(output, f, ensure_ascii=False, indent=2)

print(f'  结果已保存到 data/000021_v2_result.json')
print(f'\n  {"="*80}')
